...
m1_0 = tf.zeros((m, 1), dtype=tf.float32) # Initial state mean
...
F = tf.concat(
    [
        tf.ones((1, m), dtype=tf.float32),
        tf.eye(m, dtype=tf.float32)[1:]
    ],
    axis=0
)
...
if m == 2:
    # H = I
    H = tf.eye(2, dtype=tf.float32)
else:
    # H in reverse canonical form
    H = tf.concat(
        [
            tf.ones((1, n), dtype=tf.float32),
            tf.reverse(tf.eye(n, dtype=tf.float32), axis=[1])[1:]
        ],
        axis=0
    )
...
F_rotated = tf.zeros_like(F)
H_rotated = tf.zeros_like(H)
...
if m == 2:
    alpha_degree = 10 # Rotation angle in degrees
    rotate_alpha = tf.constant(
        alpha_degree / 180.0 * math.pi,
        dtype=tf.float32
    )
    cos_alpha = tf.cos(rotate_alpha)
    sin_alpha = tf.sin(rotate_alpha)
    rotate_matrix = tf.stack([
        [cos_alpha, -sin_alpha],
        [sin_alpha, cos_alpha]
    ])
    F_rotated = tf.matmul(F, rotate_matrix)
    H_rotated = tf.matmul(H, rotate_matrix)
...
R_structure = tf.eye(n, dtype=tf.float32)
